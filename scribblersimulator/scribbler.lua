...
function Scribbler:createBody(world)
	...
	local shape = lp.newCircleShape(32)
	self.fixture = lp.newFixture(self.body,shape)

	local wheelshape = lp.newRectangleShape(16,8)
	self.leftwheel = lp.newBody(world,self.x,self.y-32,'dynamic')
	self.leftwheelfixture = lp.newFixture(self.leftwheel,wheelshape)
	self.rightwheel = lp.newBody(world,self.x,self.y+32,'dynamic')
